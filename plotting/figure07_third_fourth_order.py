# -*- coding: utf-8 -*-
"""Third- and fourth-order process interactions with distribution insets.

Complete standalone v5_cc script. Corrected interaction arrays are plotted
directly; the fourth-order array is not reconstructed from older files. Each
panel combines a spatial field with a gray distribution sample, an emphasized
5th--95th percentile interval and median, and a Med./Neg. annotation.
"""
...
OUTPUT_STEM = OUTPUT_DIR / "third_and_fourth_order_interaction_v5_cc"
...
# Corrected third-order components and corrected fourth-order closure result.
interaction_data = [
    load_array("ts_Interact_CHF_cc.npy"),
    load_array("ts_Interact_RHF_cc.npy"),
    load_array("ts_Interact_CRH_cc.npy"),
    load_array("ts_Interact_CRF_cc.npy"),
    load_array("ts_Interact_CFHR_cc.npy"),
]
interaction_labels = [
    r"$PS_{CFH}$", r"$PS_{FHR}$", r"$PS_{CHR}$",
    r"$PS_{CFR}$", r"$PS_{CFHR}$",
]
row_letters = list("abcde")
...
fig, axes = plt.subplots(5, 3, figsize=(10.6, 8.2), sharex=True, sharey=True)
...
fig.subplots_adjust(left=0.105, right=0.897, bottom=0.105, top=0.932,
                    wspace=0.045, hspace=0.085)
fig.supxlabel("X-direction (m)", fontsize=11, y=0.050)
...
cbar.set_label("Third- and fourth-order process sensitivity index",
